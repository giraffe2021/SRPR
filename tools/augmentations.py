...
def rotation_classification_process(x):
    k = tf.random.uniform([], 0, 4, dtype=tf.int64)
    x = tf.image.rot90(x, k)
    x = random_augment(x)
    return x, tf.one_hot(k, depth=tf.cast(4, dtype=tf.int64))
...
def random_resize_and_crop_simple(x, size=(84, 84), ratio_min=0.25, ratio_max=4., max_size=1024):
    ...
    crop_size = tf.maximum(tf.cast(tf.cast(max_len, tf.float32) * ratio, tf.int64), 1)
...
def simple_augmentations(x, p=0.4):
    ...
    x_contra = tf.case([(tf.less(chance, tf.cast(p * 100, dtype=tf.int64)),
                         lambda: process(x))],
                       default=lambda: x)
...
def do_augmentations(x, p=0.5):
    ...
    @tf.function
    def process(x):
        x = random_size_ratio(x, ratio_min=0.25, ratio_max=2.25)
        x = randomFlip(tf.expand_dims(x, 0))[0]
        x = random_rotate(x)
        ...
        #
        #     tf.random.uniform([], 12, 36, dtype=tf.int64), tf.random.uniform([], 12, 36, dtype=tf.int64)))[0]

        return x

    # x_contra = tf.case([(tf.less(chance, tf.cast(p * 100, dtype=tf.int64)),
    #                      lambda: process(x))
    #                        , ((tf.greater(chance, tf.cast((1 - p) * 100, dtype=tf.int64)),
