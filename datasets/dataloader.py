...
@tf.function
def process_with_multiview(path, meta_label, global_label, way_num, episode_test_sample_num,
                           global_label_depth):
    images = tf.map_fn(read_image_with_random_crop_resize, path, dtype=tf.float32)
    x = tf.map_fn(do_augmentations, images, parallel_iterations=16)
    # x = (x - tf.cast([0.485, 0.456, 0.406], tf.float32)) / tf.cast([0.229, 0.224, 0.225], tf.float32)
    x_rotated_1, _ = tf.map_fn(rotation_process_with_limit, x, parallel_iterations=16, dtype=(tf.float32, tf.float32))
    # x_rotated_2, _ = tf.map_fn(rotation_process_with_limit, x, parallel_iterations=16, dtype=(tf.float32, tf.float32))
    x_resized_1, _ = tf.map_fn(partial(ratio_process, ratio_min=0.25, ratio_max=2.25), x, parallel_iterations=16,
                               dtype=(tf.float32, tf.float32))
    # x_resized_2, _ = tf.map_fn(partial(ratio_process, ratio_min=0.25, ratio_max=2.25), x, parallel_iterations=16,
    #                            dtype=(tf.float32, tf.float32))

    meta_label = tf.one_hot(meta_label, axis=-1, depth=way_num)
    global_label = tf.one_hot(global_label, axis=-1, depth=global_label_depth)
    return [split(images, meta_label, global_label, way_num, episode_test_sample_num),
            split(x_rotated_1, meta_label, global_label, way_num, episode_test_sample_num),
            # split(x_rotated_2, meta_label, global_label, way_num, episode_test_sample_num),
            split(x_resized_1, meta_label, global_label, way_num, episode_test_sample_num),
            # split(x_resized_2, meta_label, global_label, way_num, episode_test_sample_num)
            ]
...
class DataLoader:
    def __init__(self, data_dir_path):
        ...
        elif os.path.isfile(data_dir_path):
            pass
        else:
            raise Exception(f"非法输入！ {data_dir_path}")
    ...
    def get_dataset_V4(self, phase='train', way_num=5, shot_num=5, episode_test_sample_num=15, episode_num=None,
                       batch=1,
                       augment=False,
                       mix_up=False,
                       epochs=1,
        ...
        for ep in tqdm(range(epochs)):
            ...
            ds_episode = ds.map(partial(process_with_multiview,
                                        way_num=way_num,
                                        episode_test_sample_num=episode_test_sample_num,
                                        global_label_depth=global_label_depth),
                                num_parallel_calls=tf.data.experimental.AUTOTUNE).batch(batch,
...
if __name__ == '__main__':
    ...
    mix_up = False
    augment = True
    meta_train_ds, meta_train_name_projector = dataloader.get_dataset_V4(phase='train', way_num=ways,
                                                                         shot_num=shots,
