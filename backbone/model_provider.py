...
class MODEL(object):
    ...
    def mnist_net(self):
        ...
        model.add(Convolution2D(96, (3, 3), input_shape=input_shape, padding='same', activation='relu',
    ...
    def AlexNet(self):
        ...
        input_shape = (self.config.normal_size, self.config.normal_size, self.config.channles)
        model.add(
            Convolution2D(96, (11, 11), input_shape=input_shape, strides=(4, 4), padding='valid', activation='relu',
                          kernel_initializer='uniform'))
...
class ResnetBuilder(object):
    ...
    def build(self, config, block_fn, repetitions):
        ...
        # Classifier block
        block_shape = tf.keras.int_shape(block)
    ...
    def resnet50(self, input_shape=None, pooling=None, use_bias=True):
        if input_shape is None:
            input_shape = (84, 84, 3)
        block_fn = self.bottleneck

        input = Input(shape=input_shape)
        conv1 = self._conv_bn_relu(filters=64, kernel_size=(7, 7), strides=(2, 2))(input)
        # x = tf.nn.space_to_depth(input, 2)
        # conv1 = tf.keras.layers.ZeroPadding2D(padding=(1, 1), name='conv1_pad')(x)
        ...
        for i, r in enumerate([3, 4, 6, 3]):
            ...
            filters *= 2
            if i > 1:
                block = DropBlock2D(block_size=5, keep_prob=0.8)(block)
...
def resnet50(input_shape=None, pooling=None, use_bias=True):
    model = ResnetBuilder().resnet50(input_shape=input_shape)
    return model
#
# model = resnet50((84,84,3))
# model.summary()