...
def ResNet_12(input_shape=(84, 84, 3), pooling=False, use_bias=False,
    ...
    x1 = make_basic_block(x, use_bias=use_bias, filter_num=64, drop_rate=drop_rate,
                          pooling=True)

    x2 = make_basic_block(x1, use_bias=use_bias, filter_num=160, drop_rate=drop_rate,
                          pooling=True)

    x3 = make_basic_block(x2, use_bias=use_bias, filter_num=320, drop_rate=drop_rate, drop_block=True, dropblock_size=5,
                          pooling=True)

    x4 = make_basic_block(x3, use_bias=use_bias, filter_num=640, drop_rate=drop_rate, drop_block=True, dropblock_size=5,
                          pooling=False)

    if pooling == 'avg':
        x = tf.keras.layers.GlobalAveragePooling2D()(x4)
    elif pooling == 'max':
        x = tf.keras.layers.GlobalMaxPooling2D()(x4)
    else:
        x = x4
